...
(It is preferred to generate .wts file in the server instead of in the Jetson boxes, so as to avoid to install pytorch environment in the Jetson boxes, which would be rather complicated.)
...
(It should be run in the destination device since the optimazation is device-dependent.)
